...
\part[7] Let $T_1$ and $T_2$ be the target values in 2x1 PoW, and $\kappa$ the size of the hash function's output. What relation should they satisfy for the technique to work? Elaborate.
    ...
    % REVISAR LA DESIGUALDAD DE KAPPA
    \begin{solution} %Uncomment and type your solution here
        ...
        Para cumplir la primer prueba de double\_pow, tenemos que pedir los $\kappa - t_1$ bits más significativos del output de la función de hash deben ser $0$. Esto es porque el target $T_1$ es de $t_1$ bits, y por lo tanto, el output de la función de hash debe ser menor a $T_1$.
        ...
        Entonces, para que estas dos condiciones sean independientes, vamos a necesitar que $\kappa - t_1 + \kappa - t_2 \le \kappa$, que simplificando queda $t_1 + t_2 \ge \kappa$. Entonces, nos quedan dos "particiones" de los bits del hash.

        Vemos que, si $T_1$ y $T_2$ no fuesen potencias de dos, terminaríamos imponiendo una condición sobre la otra partición de los bits. Por ejemplo, si $T_1 = 00101000$ ($40$), $T_2 = 00000100$ ($4$) y $u=000xxxxx$ sabemos que es válido, pues es menor que $T_1$. Ahora, $u=00100xxx$ también es válido, pero el problema es que ya estamos imponiendo una condición sobre la otra partición de los bits, los que van a corresponder con la prueba del reverso. Esta prueba va a requerir que los bits menos significativos sean cero, entonces la estamos condicionando. Entonces, si $T_1$ y $T_2$ no fuesen potencias de dos, terminaríamos imponiendo una condición sobre la otra partición de los bits. Por ende, para mantener la independencia estadística entre las pruebas, vamos a necesitar que $T_1$ y $T_2$ sean potencias de dos.


        Otra cosa a notar es que hay un "balance" entre $t_1$ y $t_2$. Si uno es muy grande, el otro tiene que ser muy pequeño, y viceversa. Esto nos permite tener un PoW más fácil para uno de los dos, y un PoW más difícil para el otro.

        
        % Para que cumplir el predicado de $\texttt{double\_pow}$, tenemos que pedir los $\kappa - t$ bits más significativos del output de la función de hash deben ser $0$ (para la prueba que no usa el reverso).



        % Vamos a pedir también que $T = 2^t$, para $t$ algún entero positivo. Si no fuera así (si no fuese potencia de dos), el hecho de generar un PoW y validarlo, nos daría información sobre el otro PoW. Veámoslo con un ejemplo. Si $T = 00101000$ ($40$), y $u=000xxxxx$ sabemos que es válido, pues es menor que $T$. Ahora, si $u=00100xxx$ también es válido, pero el problema es que ya estamos imponiendo una condición sobre la otra mitad de los bits, los que van a corresponder con la prueba del reverso. Esta prueba va a requerir que los bits menos significativos sean cero, entonces la estamos condicionando.
        
        % Por esto que es que necesitamos que $T$ sea potencia de dos, para mantener la independencia estadística entre las pruebas.

        % Otra restricción, para que las pruebas sean estadísticamente independientes, es $t \ge \frac{\kappa}{2}$.
        % Veamos que la primer restricción (la que no usa el reverso) tiene como requerimiento que los primeros $\kappa - t$ bits sean cero (ya que tiene que ser menor a su target). Entonces, no son importantes los bits restantes, solo que esos sean cero.

        % Esos "bits restantes" son los que son importantes justamente para la prueba del reverso. En esta prueba, se requiere que los bits $\textbf{menos}$ significativos sean cero. Es decir, los últimos $\kappa - t$ bits.
